...
public class GymJavaHttpClient {
	...
	/**
	 * List all of the environments you started that are currently running on the
	 * server.
	 * 
	...
	public static StepObject stepEnv(String instanceId, double action, boolean isDiscreteSpace, boolean render) {
		if (isDiscreteSpace) {
			connect("/v1/envs/" + instanceId + "/step/", "POST", "{\"instance_id\":\"" + instanceId + "\", \"action\":"
					+ (int) action + ", \"render\":" + Boolean.toString(render) + "}");
		} else {
			connect("/v1/envs/" + instanceId + "/step/", "POST", "{\"instance_id\":\"" + instanceId + "\", \"action\":"
					+ action + ", \"render\":" + Boolean.toString(render) + "}");
		}
		JSONObject jobj = getJson();

		return new StepObject(jobj.get("observation"), jobj.getFloat("reward"), jobj.getBoolean("done"),
				jobj.get("info"));
	}
	...
	/**
	 * *** I COULDN'T ACTUALLY GET THIS ONE TO WORK, MAYBE MY TEST ENVIRONMENT
	 * DOESN'T USE THIS? *** Gets the name and the dimensions of the environment's
	 * observation space.
	 * 
	 * @param instanceId The id of the environment.
	...
	/**
	 * *** DIDN'T TEST! *** Flush all monitor data to disk.
	 * 
	 * @param instanceId The id of the environment.
	 */
	...
	/**
	 * *** DIDN'T TEST! *** Probably uploads your thing to OpenAI? The method just
	 * said "Flush all monitor data to disk" on the Gym HTTP API GitHub page, but it
	 * seems to do something different, I'm new to gym so I don't really know.
	 * 
	 * @param trainingDir
	 * @param apiKey
	 * @param algId
	...
	/**
	 * *** COULDN'T GET IT TO WORK! *** Attempts to shutdown the server.
	 */
	 ...
	 * 
	 * @return The JSON obtained from the connection, first line only, which should
	 *         hopefully contain all JSON.
	 */
